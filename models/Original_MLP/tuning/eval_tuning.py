...
def find_txt_files_with_prefix(directory, prefix):
    filenames = []
...
filenames = find_txt_files_with_prefix("/gpfs/project/ladan102/Ba_try/tuning", prefix="output_small_")
...
default_columns = set(superstar_var_df[superstar_var_df["Filename"] == "output_small_adam_0_1.txt"]["Features"].iloc[0])

superstar_var_df["newcol"] = superstar_var_df["Features"].apply(lambda x: list(set(x) - default_columns))

print(superstar_var_df[["newcol", "Recall", "Precision", "ROC AUC", "Macro F1 Score"]])

(superstar_var_df[["Filename", "newcol", "Recall", "Precision", "ROC AUC", "Macro F1 Score"]]).to_csv("eval_end_comb_res.csv", sep="\t", index=False)
