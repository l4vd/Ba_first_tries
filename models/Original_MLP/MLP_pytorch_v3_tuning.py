...
def upsampling(X_train, y_train, to_print):
    ...
    to_print.append("######UPSAMPLING DONE######")
...
def preprocess(df, min_max_values, exclude_cols=None):
    ...
    #to_print.append("numerical columns:", numerical_cols)
...
def load_model(model, path, to_print):
    checkpoint = torch.load(path, map_location=torch.device('cpu'))
    model.load_state_dict(checkpoint['model_state_dict'])
    to_print.append(f"Model {path} is loaded from epoch {checkpoint['epoch']} , loss {checkpoint['loss']}")
    return model


def run(additional_features, version, optim_c, lr, epochs=200, device="cpu"):
    X_test, X_train, to_print, y_test, y_train = do_setup(additional_features, device)

    # define model
    train_losses, val_losses = train_model(X_test=X_test, X_train=X_train, y_test=y_test, y_train=y_train, device=device, epochs=epochs, to_print=to_print, version=version, lr=lr, opt_choice=optim_c)

    to_print.append("######LOAD MODEL######")
    # device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = MLPClassifier(X_train.size())
    model = load_model(model, f"tuning/best_torch_{version}_model_max_val_prec.pth", to_print)
    ...
    plt.legend()
    plt.savefig(f"tuning/losses_pytorch_{version}.png")
    ...
    cm_display.plot()
    plt.savefig(f"tuning/Confusion_Matrix_pytorch_{version}.png")
    ...
    plt.savefig(f"tuning/ROC_AUC_pytorch_{version}.png")
    ...
    with open(f"tuning/output_{version}.txt", "w") as file:
...
def do_setup(additional_features, device):
    dtype_dict = {
        'song_id': str,
        'song_name': str,
    ...
    if additional_features is not None:
        if len(additional_features) > 0:
            columns_to_keep.extend(additional_features.keys())
    ...
    # to_print.append(X_train_upsampled_with_y.head())
    # prepro:
    ...
    to_print.append("######NETWORK DEFINED######")
    ...
    to_print.append("######CONVERSION TO TENSOR######")
    # Move the data to the GPU if available
    X_train = X_train.to(device)
    y_train = y_train.to(device)
    X_test = X_test.to(device)
    y_test = y_test.to(device)
    return X_test, X_train, to_print, y_test, y_train


def train_model(X_test, X_train, y_test, y_train, device, epochs, to_print, version, lr=0.001, opt_choice="adam"):
        ...
        with torch.no_grad():
            ...
            if epoch_val_loss < best_val_loss:
                ...
                torch.save({
                    ...
                    'loss': epoch_val_loss,
                }, f'tuning/best_torch_{version}_model_min_val_loss.pth')
            if epoch_val_acc > best_val_acc:
                ...
                torch.save({
                    ...
                    'optimizer_state_dict': optimizer.state_dict(),
                    'loss': val_loss,
                }, f'tuning/best_torch_{version}_model_max_val_acc.pth')
            if epoch_val_prec > best_precision or epoch == 1:
                ...
                }, f"tuning/best_torch_{version}_model_max_val_prec.pth")

            val_prec.append(epoch_val_prec)
            val_losses.append(epoch_val_loss)
            val_accs.append(epoch_val_acc)
            print(f"Epoch [{epoch + 1}/{epochs}], Training Loss: {avg_epoch_train_loss:.4f}, Validation Loss: {epoch_val_loss:.4f}, Validation Accuracy: {epoch_val_acc:.4f}, Validation Precision: {epoch_val_prec:.4f}")
            to_print.append(
...
if __name__ == "__main__":
    ...
    for lr in learning_rate:
        for optim_c in optim_choice:
            i += 1
            num_str_without_dot = str(lr).replace('.', '_')
            version = "small_" + optim_c + "_" + num_str_without_dot
            # addit = {"superstar_x": int, "superstar_v1_x": float}
            run(input_dict, version=version, optim_c=optim_c, lr=lr, epochs=20, device=device)
            print("###Done " + str(i) + "/" + str(len(learning_rate) * len(optim_choice)) + "###")
