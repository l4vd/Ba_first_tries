...
def run(additional_features, version, epochs=200, device=torch.device("cpu"), input_file="../data_superstar_v1_0.csv" ):
    dtype_dict = {
        'song_id': str,
        'song_name': str,
    ...
    data = pd.read_csv(input_file, delimiter=",", dtype=dtype_dict, na_values=[''])
...
if __name__ == "__main__":
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    input_file = "../data_superstar_v1_0_alt.csv"
    spec = "alt"
    for i in range(2 ** 3):
        ...
        print(string_rep)
        if string_rep[0] == '1':
            input_file = "../data_superstar_v1_0.csv"
            spec = "norm"
        elif string_rep[0] == '0':
            input_file = "../data_superstar_v1_0_alt.csv"
            spec = "alt"
    ...
    for i in range(6, 2 ** 3):  #only last one
        ...
        # addit = {"superstar_x": int, "superstar_v1_x": float}
        run(input_dict, f"pagerank_{spec}_{i}", epochs=200, device=device, input_file=input_file)
